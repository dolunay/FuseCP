$(document).ready(function () {
    ...
    function initPopover($collection, options) {
        if (!window.bootstrap || !window.bootstrap.Popover) {
            return;
        }

        forEachElement($collection, function (element) {
            window.bootstrap.Popover.getOrCreateInstance(element, options || {});
        });
    }

    function initTooltip($collection, options) {
        if (!window.bootstrap || !window.bootstrap.Tooltip) {
            return;
        }

        forEachElement($collection, function (element) {
            window.bootstrap.Tooltip.getOrCreateInstance(element, options || {});
        });
    }
    ...
    initPopover($('#style-switcher li[data-bs-toggle="popover"]'), {
        html: true
    });
    ...
    $.fn.bootstrapSwitch.defaults.labelWidth = '3';
    $(".checkbox-toggle").bootstrapSwitch();
